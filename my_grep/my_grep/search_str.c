...
bool search_str(char* new_line, char* search_phrase, regex_member** member_list, int option_flag)
{
  ...
  if (_x == (_x & option_flag)) {
    if (_E == (_E & option_flag)) {
      ...
      if (strcmp(line_copy, phrase_copy) == 0 || strcmp(line_copy, strcat(phrase_copy, "\n")) == 0)
        return_val = true;
      else
        return_val = false;
    }
      ...
      switch (member_list[i]->type) {
        ...
        case bracket:
          if (test_char >= member_list[i]->p_square_bracket->start_char &&
              test_char <= member_list[i]->p_square_bracket->end_char)
            loop_condition = true;
          else
            return false;
          break;
...
int resolve_parentheses(char* test_str, regex_member** member_list, int index)
{
  ...
  if (strlen(test_str) < len_a && strlen(test_str) < len_b)
    return 0;  // test_str too short can't match
