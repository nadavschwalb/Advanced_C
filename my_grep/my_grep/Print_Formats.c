...
void print_formater(FILE* input_file, char* line_buffer, formating_args* args, int options_flag)
{
  ...
  if (formated_line == NULL) {
    printf("failed to allocate memory\n");
    exit(-1);
  }
  if ((_b & options_flag) == _b && (_n & options_flag) == _n) {
    sprintf(formated_line, "%d%c%d%c%s", args->line_num, args->seperating_char, args->byte_num, args->seperating_char,
            line_buffer);
...
void printer(FILE* input_file, search_args* search_args, formating_args* format_args, regex_member** member_list,
             int options_flag)
{
